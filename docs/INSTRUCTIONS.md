...
- 기간 필터: 7 / 30 / 90일. 단, 추이 블록은 기간 필터와 분리해 최근 6개월로 고정한다
- 부서 필터: 전체 / 특정 부서. 인기 콘텐츠는 실행자 부서, 부서 확산은 콘텐츠 등록 부서를 기준으로 적용한다
- 인기 콘텐츠 블록에서는 프롬프트/에이전트를 구분 표시하되, 하나의 목록으로 합쳐 순위를 매긴다
- 팀별 AI 구독 현황은 이번 단계에서 시드로 관리하며 별도의 편집 UI를 만들지 않는다
...
- 기간 필터는 추이를 제외한 활용 지표에 반영되고, 추이는 항상 최근 6개월을 표시한다
- 부서 필터가 모든 블록에 반영된다
...
- [ ] 기간 필터를 7 → 30 → 90으로 바꾸면 추이를 제외한 기간 기반 수치가 함께 변하는가
- [ ] 추이는 기간 필터와 무관하게 최근 6개월을 유지하는가
